...
``` python
qq_plot(analysis_df.loc[:, 'norm_auc_omv'], title = 'QQ Plots for OMV')
...
print(f"Baseline risk: {or_const}")
if pval_lr < 0.05:
...
```
                  ...
                  lower  upper
    const        0.9793 0.9930
    norm_auc_omv 1.0074 1.0214
    The estimated odds for pulmonary_embolism change by 1.0144 [1.0074, 1.0214] for each 10% increase of norm_auc_omv (accuracy: 0.5311, p-value: 0.1635).
    Baseline risk: 0.8638 [0.9793, 0.9930]
    No significant influence of normOMV on the chances of postop Pulmonary embolism.

![](8_Confounding_bias_files/figure-commonmark/cell-8-output-2.png)

LogReg PE~Age

``` python
qq_plot(analysis_df.loc[:, 'age'], title = 'QQ Plots for age')
...
if pval_lr < 0.05:
    if odds_ratio < 1: # decreased odds
        print(f"Each additional year decreased the chances of postop {config.get('long_names').get('pulmonary_embolism')} by {(1-odds_ratio)*100:.2f} %.")
    else: # increased odds
        print(f"Each additional year increased the chances of postop {config.get('long_names').get('pulmonary_embolism')} by {(odds_ratio-1)*100:.2f} %.")
else:
...
```
    ...
    const 0.2030 0.2313
    age   1.0262 1.0285
    The estimated odds for pulmonary_embolism change by 1.0273 [1.0262, 1.0285] for each one unit increase of age (accuracy: 0.5532, p-value: 0.0000).
    Baseline risk: 0.2167 [0.2030, 0.2313]
    Each additional year increased the chances of postop Pulmonary embolism by 2.73 %.

![](8_Confounding_bias_files/figure-commonmark/cell-9-output-2.png)

LogReg PE~BMI

``` python
qq_plot(analysis_df.loc[:, 'bmi'], title = 'QQ Plots for BMI')
...
if pval_lr < 0.05:
    if odds_ratio < 1: # decreased odds
        print(f"Each unit increase of BMI decreased the chances of postop {config.get('long_names').get('pulmonary_embolism')} by {(1-odds_ratio)*100:.2f} %.")
    else: # increased odds
        print(f"Each unit increase of BMI increased the chances of postop {config.get('long_names').get('pulmonary_embolism')} by {(odds_ratio-1)*100:.2f} %.")
else:
...
```
    ...
    const 0.1043 0.1270
    bmi   1.0829 1.0913
    The estimated odds for pulmonary_embolism change by 1.0871 [1.0829, 1.0913] for each one unit increase of bmi (accuracy: 0.6259, p-value: 0.0000).
    Baseline risk: 0.1151 [0.1043, 0.1270]
    Each unit increase of BMI increased the chances of postop Pulmonary embolism by 8.71 %.

![](8_Confounding_bias_files/figure-commonmark/cell-10-output-2.png)
